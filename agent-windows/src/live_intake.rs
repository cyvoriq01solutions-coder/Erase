...
//! stick or a charger. It is read-only: it never writes to a volume and it
//! never runs `powercfg`. Camera preview stays in the webview.
//!
//! Charging is not a CG-1.0 scoring domain. A sensed USB volume is not a
//! substitute for the four physical-port attestation points.
...
/// Map a Win32_Battery BatteryStatus code. Used by the overlay and by tests.
#[must_use]
...
pub fn probe_live_intake() -> LiveIntakeProbe {
    LiveIntakeProbe {
        removable: removable_from_targets(list_scan_targets()),
...
mod tests {
    use super::{
        LiveIntakeNotes, interpret_battery_status, parse_power_stdout, removable_from_targets,
    };
